                            ...
                            int question_len, const char *ipv6)
{
    ...
    rr->class = htons(DNS_CLASS_IN);
    rr->ttl = htonl(300);
    rr->rdlength = htons(16); // IPv6地址长度为16字节

    // 将IPv6字符串转换为二进制格式
    if (inet_pton(AF_INET6, ipv6, rr->rdata) != 1)
    {
        // 转换失败，返回错误
        return -1;
    }

    // 计算响应包长度：头部 + 问题区 + 资源记录头部 + IPv6地址(16字节)
    return sizeof(DNSHeader) + question_len + 10 + 16; // 10 = RR头部字段长度
}