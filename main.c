...
// 全局上游DNS服务器IP
static char g_upstream_dns_ip[64] = DEFAULT_UPSTREAM_DNS_IP;
...
#ifdef _WIN32
    context->upstream_addr.sin_addr.s_addr = inet_addr(g_upstream_dns_ip);
#else
    inet_pton(AF_INET, g_upstream_dns_ip, &context->upstream_addr.sin_addr);
#endif

    print_debug_info("DNS服务器启动，监听端口 %d\n", MY_PORT);
    if (g_debug_mode)
    {
        print_debug_info("上游DNS服务器: %s\n", g_upstream_dns_ip);
    }
    return 0;
}
...
int main(int argc, char *argv[])
{
    ...
    parse_command_line(argc, argv, &dns_server, &config_file);

    // 设置上游DNS服务器IP
    strncpy(g_upstream_dns_ip, dns_server, sizeof(g_upstream_dns_ip) - 1);
    g_upstream_dns_ip[sizeof(g_upstream_dns_ip) - 1] = '\0';
